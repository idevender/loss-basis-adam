"""
Post-ladder diagnostics (Appendix C8).

A) signum horizon-dependence: n in {40, 128}, cosine horizon in {20k, 40k, 80k} plus a constant-lr
   control, with (recovery, effective-rank, balancedness, train) trajectories every 500 steps.
B) budget arm: GD / adam_p0rms / adam at n in {128, 256}, one lr notch below the ladder's edge
   selection, budget 150k, testing whether the equivariant rows' degradation with n is flow-limit
